...
6. 针对长PRD单轮JSON易截断的问题，实现稳定陈述ID紧凑分析：LLM只返回分类ID，Python回填原文并校验来源；整份需求统一审核待确认问题。
...
| 长PRD需求分析 | 2735字符、2片段 | 13.38秒完成，3次调用、0次拆分、未发生JSON截断 | 紧凑ID与非思考结构化调用在当前单样本有效；仍需扩样验证语义质量 |
| 长PRD完整Agent链路 | 2735字符、真实LLM/Ollama/Milvus/MySQL | 约416秒执行到2轮修正上限；Embedding成功、Milvus命中1条、无JSON截断，恢复后未重复执行 | 最终82分仍有未覆盖项，因此等待人工反馈而非生成报告 |
...
- 不能写“任意长PRD都能快速稳定处理”：13.38秒结果只来自1份2735字符合成PRD，尚未覆盖更长文档、复杂图片和更多业务领域。
