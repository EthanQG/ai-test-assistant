...
重复页面验收时`chunk-002`仍出现输出截断。本轮增加三层保护：

1. 所有`response_format=json_object`调用固定使用`temperature=0`，普通文本仍保留配置温度；
2. RequirementAnalyzer单片段输出限制为摘要100字、事实40条、规则30条、状态20条、风险12条和问题5条，并要求合并同义表达；
3. 仅在明确命中`max_tokens`、`finish_reason=length`或“JSON被截断”时，将失败片段减半后递归重试，最多3层、最小250字符；
4. 超时、网络和普通字段校验错误不会触发自适应拆分；
5. STEP_COMPLETED事件记录实际尝试次数与自适应拆分次数；
6. 真实Pro复验在480秒内未完成；Flash以8次尝试、2次自适应拆分在319.55秒完成；
7. 结论：失败保护可让Flash完成任务，但当前同步长PRD耗时不可接受，Pro不适合该演示链路；不能将本阶段描述为性能优化成功。
...
当前不应继续用“重复长文本生成”的方式提高上限。下一步需要在继续开发前评估更紧凑的输出契约：

1. Python先为原文陈述分配稳定statement_id；
2. LLM只返回事实/规则/状态对应的ID和少量风险、问题，Python回填原文，减少输出Token；
3. 先用同一PRD验证调用次数与耗时，再决定是否增加有限并行；
4. 在紧凑契约完成前，不继续进行页面完整流程验收。
