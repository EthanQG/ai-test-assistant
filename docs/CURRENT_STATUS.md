...
## 当前阶段：2.18.2 Web待确认问答与视觉层级（已完成）

原生Web前端在任务进入`waiting_for_user`后读取同一任务的`open_questions`，动态生成答案输入和“暂不确定”选项。提交时校验每个问题，调用`/clarifications`后重新调用`/run`，继续轮询同一`task_id`。

视觉上增加五阶段流程条、柔和页面背景、轻量阴影和独立问答卡片，保持原双栏和原生三文件结构。页面仍只调用公开API，不修改后端状态机。浏览器自动检查工具当前缺少运行资产，完整浏览器验收留到前端主要功能完成后统一执行。
...
下一阶段2.18.3接入结构化测试点列表、分页和详情展示；暂不同时实现人工反馈和历史任务。
