...
已新增`docs/RESUME_EVIDENCE.md`，集中记录可以写入简历的能力、已有量化证据、禁止夸大的结论、推荐简历表述和面试问题。后续默认冻结功能开发，优先用于项目学习、演示和面试准备：

1. 录制或准备一次完整演示；
2. 按`LEARNING_NOTES.md`逐模块复盘核心设计；
3. 完整10×3和语义评分作为可选增强，不阻塞投递；
