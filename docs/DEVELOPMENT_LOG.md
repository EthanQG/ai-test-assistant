...
| 阶段 2.18.16 | 已完成 | 宽屏工作区、历史标题两行展示与修正上限轮询出口 | 本次提交 |
| 阶段 2.18.17 | 已完成 | 产品展示名统一与桌面工作区10px边距收尾 | 本次提交 |
| 阶段 2.19.1 | 已完成 | 知识资产摘要搜索、状态筛选、分页与详情API | 本次提交 |
| 阶段 2.19.2 | 已完成 | 独立知识库页面、一级导航与资产详情浏览 | 本次提交 |
| 阶段 2.19.2.1 | 已完成 | 强化任务工作台与知识库一级导航 | `c183ffc` |
| 阶段 2.19.3 | 已完成 | 资产停用、恢复、索引重试与错误展示 | 本次提交 |
...
## 阶段 2.18.16：宽屏历史栏与修正上限展示修复

### 问题与根因

- 桌面工作区使用`min(1460px, ...)`，大屏两侧留下较多空白，210px历史栏还要为两个操作按钮预留空间，标题只能显示很短的一行；
- 达到自动修正上限属于Orchestrator的受控停止动作，领域状态仍保留`running`以接受人工反馈。后台已经`stopped`，但Web只把`completed/failed/waiting_for_user`作为轮询出口，导致页面误显示为仍在执行。

### 修复内容

- 桌面工作区使用浏览器可用宽度，历史栏增至240px，标题最多显示两行；
- API进度Presenter把`revision_limit_reached`确定性映射为等待人工反馈文案；
- 前端遇到该标记后停止轮询、取消执行态并打开人工反馈Tab；
- 不修改AgentState、Orchestrator动作、Reviewer门槛和自动修正上限。

## 阶段 2.18.17：产品命名与页面边距收尾

- 用户界面、浏览器标题、README、当前PRD和FastAPI文档统一使用“AI 测试分析助手”；
- Agent仍是内部受控编排架构，不因产品名调整而改变技术定位；
- 桌面工作区由两侧16px调整为两侧10px留白，兼顾宽屏利用率与视觉边界；
- 未修改Agent流程、API路径、页面交互和数据结构。

## 阶段 2.19.1：知识资产摘要查询边界

### 本阶段目标

为独立知识库管理页面建立低成本、可分页的数据读取入口，不让列表查询加载所有完整知识资产快照。

### 修改内容

- Repository新增`KnowledgeAssetSummaryPage`契约，支持关键词、状态、offset和limit；
- InMemory与MySQL实现统一语义，MySQL只查询已有摘要列；
- Application Service返回只读摘要View；
- FastAPI新增`GET /api/v1/knowledge-assets`和`GET /api/v1/knowledge-assets/{asset_id}`；
- 增加Repository、Application Service和API测试。

### 设计原因与边界

知识资产完整快照包含原需求、测试点、评审和报告，不适合列表批量读取。列表负责低成本定位资产，详情负责按ID读取单条资产。本阶段没有新增管理页面，也没有实现下线、删除和索引重试。

### 验证结果

相关模块62项测试通过；阶段完成时继续执行全量pytest、compileall和diff检查。

### 下一步计划

阶段2.19.2增加独立知识库页面，复用本阶段API展示摘要列表和资产详情。

## 阶段 2.19.2：独立知识库浏览页面

### 本阶段目标

让用户在不挤压任务工作台的前提下，浏览已经确认保存的历史测试资产。

### 修改内容

- 顶部新增“任务工作台 / 知识库”一级导航；
- 新增独立`knowledge.html`和`knowledge.js`；
- 左侧支持关键词搜索、索引状态筛选和分页；
- 右侧展示评分、测试点数量、资产版本、结构化需求、测试点折叠详情和最终报告；
- 详情View补充完整知识资产内容，但仍由Application Service隔离Repository。

### 设计边界

知识库是跨任务的资产管理空间，不继续嵌入单个任务的报告Tab。当前页面只读，避免在未完成管理语义和Milvus联动前提供危险删除入口。

### 验证结果

新增原生页面与接口契约测试；全量回归、浏览器截图和交互验收在本阶段提交前完成。

页面实测发现空状态元素的`display:grid`覆盖了原生`hidden`，导致详情被推到一个视口之后。补充全局`[hidden]`显示规则和静态回归测试后，空状态与详情只会展示其中一个。当前5条内容较少的资产是离线评测种子，不代表正式KnowledgeAsset字段被裁剪。

用户二次反馈仍命中旧表现后，页面进一步加入静态资源版本参数、显式`display`切换及首条资产自动选中，避免浏览器缓存或样式优先级再次让空状态占位。

阶段2.19.2.1将顶部文字链接强化为分段导航，使用完整边框、按钮热区和蓝色选中态，不改变页面信息架构。

### 下一步计划

阶段2.19.3评估并实现资产停用、索引失败重试等明确管理动作，物理删除仍需单独确认数据语义。

## 阶段 2.19.3：知识资产受控管理

### 本阶段目标

提供可恢复、可审计的知识资产管理动作，而不是直接物理删除权威数据。

### 修改内容

- 已索引资产支持停用，MySQL先保存`retired`状态，再清理Milvus向量；
- 已停用资产支持恢复，通过`pending_index`重新构建向量并回到`indexed`；
- `index_failed`资产支持使用唯一请求ID重试，并复用既有重试审计表；
- 详情接口和页面展示最近索引错误；
- 页面根据资产状态只显示当前合法操作。

### 设计原因

停用既能立即阻止RAG召回，又保留版本、来源任务和审计记录。直接删除需要跨MySQL和Milvus处理不可恢复的一致性问题，对当前V1价值有限，因此不开放。

### 验证结果

Application Service、API和页面专项50项测试通过；完成前执行全量pytest、compileall和差异检查。

### 下一步计划

优先进行真实页面验收和项目学习；知识资产物理删除、批量操作和复杂权限不作为当前秋招P0。

## 阶段 2.20.1：真实感图文PRD评测样本

### 本阶段目标

解决原有图文夹具过短、只验证单一解析能力的问题，提供可直接上传项目体验且能重复评分的综合PRD。

### 修改内容

- 新增智能门锁近场自动解锁DOCX，包含正文、规则矩阵、异常边界、流程图和3个待确认主题；
- 新增电商订单履约与优惠结算DOCX，包含金额规则表、库存/幂等/支付规则、结算页UI原型和3个待确认主题；
- 新增`realistic_prd_v1.json`金标准，记录关键文本、表格数、图片数、视觉类型和待确认主题；
- 新增真实`DocumentService`解析测试，证明两份文档的正文、表格和图片进入统一`DocumentContent`。

### 设计边界

样本完全虚构且脱敏，可以提交GitHub。确定性测试只证明解析边界没有丢失正文、表格和图片；没有配置真实视觉模型时，不能证明流程分支和UI交互语义识别准确。两份综合样本用于体验和解析验收，不替代10份需求分析金标准。

### 验证结果

新增样本专项11项测试通过；阶段结束前继续执行全量pytest、compileall和差异检查。DOCX结构已由`python-docx`和真实解析器检查；当前环境缺少LibreOffice，无法完成DOCX渲染截图复核，需在本机Word或装有LibreOffice的环境人工查看版式。

真实页面体验发现人工反馈恢复存在一处前端竞态：反馈提交后后台已经排队或运行，但旧评审仍携带修正上限标记，轮询函数因此提前退出。修复后仅当后台不再`queued/running`时才按修正上限停止；Agent状态、修正次数和Orchestrator均未修改。

### 下一步计划

先用原生Web上传两份样本体验完整任务，再按是否配置视觉端点分别记录降级行为和真实视觉语义指标。

## 阶段 2.20.2：可选人工反馈与风险报告

### 本阶段目标

让人工反馈成为报告完成后的可选质量增强，而不是自动修正用尽后的强制阻塞步骤。

### 修改内容

- Orchestrator在Reviewer未通过且自动修正达到上限时转入Finalizer；
- Finalizer允许该特定状态生成报告，并写入未达质量门槛的明确警告；
- 未评审或仍可自动修正的失败结果继续禁止最终化；
- 完成后的现有人工反馈入口继续通过`reopen_for_feedback`重开同一任务并生成新版报告；
- 前端只为Reviewer通过的完成任务展示知识库沉淀入口，后端准入门禁保持不变。

### 设计原因

Reviewer分数是质量建议，不应让用户在已有测试点可用时无法获得报告。自动修正上限仍然限制模型循环，但终点变为“带风险的可用报告”。知识库继续采用更严格门禁，把可下载和可复用两种质量等级分开。

### 验证结果

核心专项53项测试通过；阶段结束前执行全量pytest、compileall和差异检查。

### 下一步计划

阶段2.20.3对质量评审问题进行确定性分类、排序和编号展示，不改变Reviewer评分逻辑。

### 体验验收修复：Reviser重复新增

真实电商PRD执行时，模型尝试`add`一个与现有测试点同名的条目。重复标题校验原本在`generate_and_parse_json`之后执行，因此无法使用已经存在的一次结构化重试，任务直接失败。

本次把修正范围校验和`apply_to`原子合并校验放入同一个解析回调。第一次出现重复标题、无效目标或无实际变化时，模型获得一次受控重试；第二次仍不合法才按原规则失败。整个集合仍只在全部操作验证成功后写入State，没有放宽重复标题约束。

### 体验验收修复：最终质量摘要快照兼容

2.20.2为风险报告的`quality_summary`增加了`review_passed`，但schema v1反序列化器仍按旧字段集合做严格校验。结果是MySQL任务写入和摘要查询正常，完整快照读取却因未知字段失败，页面进度接口持续返回500。

本次将`review_passed`纳入最终质量摘要的可选布尔字段：新快照完整往返，旧快照缺少字段仍可恢复，字符串等非法类型和其他未知字段继续拒绝。没有修改Agent状态机、报告生成规则或MySQL表结构。

## 阶段 2.20.3：端到端验收与展示收尾

### 本阶段目标

用真实外部服务完成一轮用户操作验收，并只修复阻碍展示收尾的问题，不再扩展Agent业务能力。

### 验收链路

- 上传真实PDF并解析需求；
- 回答3个待确认问题后恢复同一`task_id`；
- 完成Milvus检索、测试点生成、Reviewer评审和报告整理；
- 在报告完成后可选提交人工反馈，再次经过Reviser、Reviewer和Finalizer；
- 将反馈前后结果分别保存为知识资产V1和V2；
- 重启FastAPI后从MySQL恢复任务，终态再次推进不新增事件；
- 验证历史任务重命名和删除接口。

### 收尾修改

- 进度Presenter从`review_result.overall_score`读取正式Reviewer评分，并兼容旧`score`字段；
- 质量评审问题和建议由无序列表改为有序列表；
- 补充报告Markdown Blob、统一任务文件名和前端静态资源版本的契约测试；
- 同步schema v1对`quality_summary.review_passed`的向后兼容支持。

### 实测结果

真实订单任务首次生成10个测试点、Reviewer 88分；提交一条可选人工反馈后生成12个测试点和新版报告。知识库保存V1与V2且均完成索引。主任务累计节点耗时97.18秒，其中LLM累计81.56秒、Milvus检索15.59秒；该数据只代表本次本机与当前外部服务环境，不作为通用性能结论。

### 下一步计划

冻结当前功能范围，进入README、简历项目描述、演示脚本和分模块学习整理。

## 阶段 2.20.4：README展示与项目冻结

### 本阶段目标

把根目录README调整为招聘方和面试官能够快速理解的项目首页，并正式停止业务功能扩张。

### 修改内容

- 首屏明确产品问题、受控Agentic Workflow定位和当前范围；
- 使用流程图说明Web、FastAPI、Application Service、Orchestrator、Repository、MySQL与Milvus的依赖方向；
- 区分MySQL权威快照与Milvus向量候选的存储职责；
- 写入真实端到端验收链路和谨慎限定的单次性能数据；
- 简化启动步骤，提供可公开体验的PDF/DOCX样本入口；
- 明确已知限制和不能写进简历的夸大表述；
- 将详细开发历史继续留在docs，避免README成为阶段日志。

### 下一步计划

按模块开始代码学习，随后产出简历项目描述、演示脚本和面试问答。
