...
Reviser不是只返回“修改了什么”，而是返回修正后的完整测试点集合。测试点越多，
...
现在将“大体量结构化输出”预算统一为
`LARGE_STRUCTURED_OUTPUT_MAX_TOKENS = 8192`，并用于Generator、Reviewer和
Reviser。单元测试通过Fake LLM记录`max_tokens`，验证每个节点确实把8192传到
`generate_json()`。

#### 问：为什么不让Reviser只返回差异内容以节省token？

> 差异协议确实更省token，但需要额外设计测试点稳定ID、增删改操作模型、冲突校验和原子应用规则。当前阶段返回完整集合更容易做严格结构校验，也能保证State不会处于“只应用了一半修改”的状态。统一8192是当前小阶段的稳定性修复，后续测试点规模继续增长时，再评估差异修正协议。
