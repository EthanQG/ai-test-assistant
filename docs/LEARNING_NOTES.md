...
## 49. 阶段2.15.6：ContextBuilder与节点输入预算

### 49.1 为什么不能把完整AgentState发给每个节点

不同节点的职责不同。需求分析需要原始需求和用户补充；知识检索需要需求摘要、事实、规则和风险；
Reviewer需要结构化需求和测试点，但不需要最终报告、完整事件或页面状态。完整State会增加Token、等待时间，
还会让模型受到无关历史干扰。

### 49.2 ContextBuilder做什么
...
→ 限制可裁剪长文本
→ 检查输入预算
→ 返回节点上下文和context_metrics
→ PromptService构造最终Prompt
```

它不决定下一个节点，也不调用LLM。它只是一个“节点上下文装配器”。Orchestrator仍负责流程，PromptService
仍负责提示词格式，LLM仍负责理解、生成和评审。

### 49.3 为什么Token只是估算

当前本地估算把中日韩字符大致按1 Token、其他非空字符大致按4个字符1 Token计算。不同模型Tokenizer
结果不同，所以该值只能用于提前控制规模，不能作为实际计费或性能结论。模型API返回的usage才属于真实Token证据。

### 49.4 为什么不自动压缩所有内容

自动摘要本身会增加一次LLM调用，并可能丢失“5000元”“最多重试2次”“失败后转人工审核”这类关键事实。
当前优先使用确定性裁剪并保护数字、规则、状态和来源。测试点、评审反馈等结构化信息如果放不下会明确失败，
不会悄悄删掉一部分后继续评审。

### 49.5 和执行时间的关系

ContextBuilder不减少Agent节点数量，也不改变模型服务速度。它减少的是不必要输入，因此对长PRD和大段RAG内容
更可能有帮助。到底节省多少时间必须由2.15.7的真实耗时埋点和2.16的对照实验回答，当前不能声称已经明显提速。

### 49.6 面试问题与参考答案

**问题：ContextBuilder和PromptService有什么区别？**

参考答案：ContextBuilder决定某节点能看到哪些业务数据以及最多看到多少；PromptService决定这些数据如何组织成
System/User Prompt。前者控制上下文边界，后者控制提示词表达。

**问题：为什么超预算时不直接截断测试点数组？**

参考答案：截断会导致Reviewer误以为已经评审完整集合，属于静默数据丢失。第一版选择明确失败，后续只有在设计出
可验证的分批评审和聚合规则后，才考虑安全拆分。

### 49.7 动手练习

1. 阅读五个`build_*`方法，列出每个节点能看到的字段；
2. 把需求字符上限临时调小，观察关键数字和裁剪标记是否保留；
3. 给Reviewer构造超预算测试点，确认抛出`ContextBuildError`；
4. 修改ContextBuilder返回的测试步骤，确认不会改变AgentState原值。

### 49.8 掌握检查

- [ ] 能区分Orchestrator、ContextBuilder和PromptService的职责
...
## 51. 阶段2.16.1：先定义答案，再评价模型

### 51.1 为什么需要人工金标准

如果只让另一个LLM评价生成结果，就无法确定评分模型是否也在编造。离线评测先由人阅读脱敏需求，明确哪些事实、规则、
风险和场景是正确答案，哪些业务结论绝对不能出现。模型输出随后与同一份金标准比较，方案之间才有共同尺度。
...
→ 校验schema版本、字段、类型和case_id
→ EvaluationDataset / EvaluationCase / GoldAnnotations
→ 后续评测Runner
```

`schema_version`用于控制标注文件结构。严格拒绝未知字段可以及时发现拼写错误，避免某条人工标注悄悄没有参与评分。

### 51.3 六类标注分别解决什么

- 事实：检查模型有没有正确读取原文；
- 规则：检查关键阈值和状态约束是否保留；
- 风险：检查测试分析有没有覆盖重要失败可能；
- 关键问题：检查需求信息不足时是否问对问题；
- 必要场景：检查测试点是否覆盖核心、边界和异常路径；
- 禁止断言：统计模型擅自补充了多少没有依据的业务规则。

### 51.4 为什么先写3份、再扩充到10份

先用3份种子样例验证格式，再沿用同一契约扩充到10份，比直接写20份再返工成本更低。当前10份已经由用户逐项修订并接受，
因此标记为`review_status=reviewed`。这只代表单人复核完成，不代表模型质量提高，也不提供双人标注一致性证据。

### 51.5 面试问题与参考答案

**问题：为什么禁止断言不能只靠关键词匹配？**

参考答案：同一个无依据结论可能有多种表达方式，关键词只能完成确定性初筛，最终语义判断仍需要人工规则或经过验证的评分方法。
当前阶段只建立金标准，后续评分脚本会明确哪些指标可自动计算、哪些必须人工复核。

**问题：为什么评测数据模型要不可变？**

参考答案：金标准是对照基准。运行某个方案时如果意外修改了标注，后续方案就不再使用相同尺度。冻结dataclass并将列表转换为tuple，
可以降低实验过程污染金标准的风险。
...
- [ ] 能解释人工金标准与模型输出的区别
- [ ] 能说出六类标注各自对应的质量问题
- [ ] 能解释为什么10份单人复核金标准仍不能直接证明模型效果
- [ ] 能说明schema版本、严格字段校验和不可变对象的价值

## 52. 阶段2.16.2：先准备可重复的图文评测样本

### 52.1 为什么没有真实PDF也能先开始

真实公司PRD通常包含敏感业务信息，不能直接提交到个人仓库。项目先用脚本生成完全虚构的PDF、DOCX、流程图和UI图，
把评测文件格式、金标准结构和验证流程跑通。以后拿到合规脱敏样本时，可以沿用同一入口替换或扩充数据。

### 52.2 五份样本分别检查什么

- 原生PDF：检查已有文本层能否被直接提取；
- DOCX权限矩阵：检查正文和表格单元格关系能否保留；
- 扫描PDF：故意不包含文本层，用来触发OCR路径；
- 订单流程图：检查节点、连线和分支条件；
- 发票上传UI图：检查控件、提示、文件状态和限制信息。

### 52.3 文件和金标准的关系

样本文件相当于“考卷”，`gold_v1.json`相当于“标准答案”。例如流程图图片本身只包含像素，金标准明确列出
“校验库存→创建订单（库存充足）”这条关系。后续Runner把解析结果与这些答案比较，才能计算召回和准确性。

### 52.4 为什么当前还不能说解析效果很好

现在的测试只确认样本本身有效：原生PDF确实有文字层、扫描PDF确实没有文字层、DOCX表格结构正确、PNG能够打开。
还没有把真实解析输出与金标准批量比较，也没有真实视觉模型结果，所以不能写“图文解析准确率达到某个数值”。

### 52.5 面试问题与参考答案

**问题：为什么要同时准备原生PDF和扫描PDF？**

参考答案：两者走不同路径。原生PDF应优先读取文本层，速度快且文字准确；扫描PDF只有图片，需要OCR。混在一起评测会看不出
错误来自文本抽取还是OCR，所以必须分样本统计。

**问题：合成数据有什么价值和限制？**

参考答案：价值是安全、可重复、答案明确，适合验证评测链路；限制是版式和噪声比真实PRD简单，不能代表真实分布。
因此先用合成数据建立底座，之后再补充合规脱敏样本。

### 52.6 掌握检查

- [ ] 能解释样本文件与金标准的区别
- [ ] 能说明原生PDF和扫描PDF为什么需要分开评测
- [ ] 能说出5份样本各自覆盖的解析能力
- [ ] 能说明为什么当前仍不能声称图文解析效果已达标

## 53. 阶段2.16.2：把解析结果和标准答案比较

### 53.1 三个指标如何理解

- 正文行召回率：金标准中的句子有多少被解析结果完整覆盖；
- 字符准确率：去掉空白后，用编辑距离衡量字符替换、删除和新增；
- 表格单元格准确率：按相同行列位置比较每个单元格。

例如金标准有5行，解析结果缺少1行，正文行召回率就是`4 / 5 = 0.8`。报告同时保存缺少的具体句子，
这样开发者不只看到一个分数，还知道问题在哪里。

### 53.2 为什么OCR不能直接评分to_plain_text

`DocumentContent.to_plain_text()`为了来源追踪，会给OCR文字加上来源ID和置信度。如果把这些标签也作为识别文字评分，
会把正确OCR结果错误地判低。因此Runner只读取`DocumentOcrElement.text`，来源信息仍保留在领域对象和报告警告中。

### 53.3 为什么不计算综合总分

正文漏一条规则、表格错一个金额和OCR错一个字符的业务影响不同。当前样本很少，强行加权只会制造一个看似直观但没有依据的数字。
所以报告逐样本、逐指标展示，等真实样本和评分需求稳定后再决定是否需要汇总。

### 53.4 面试问题与参考答案

**问题：当前评测结果都是1.0，能否说明解析器准确率100%？**

参考答案：不能。它只说明3份简单合成样本在当前环境中全部命中金标准。真实PRD还有复杂排版、低清扫描、跨页表格和图片噪声，
必须增加合规脱敏样本后才能报告更有代表性的结果。

**问题：为什么字符准确率忽略空白？**

参考答案：中文OCR经常在词语之间插入额外空格，这通常不改变需求语义。忽略空白可以避免版式差异干扰文字质量；
文字替换、缺失和新增仍会被编辑距离统计。

### 53.5 掌握检查

- [ ] 能解释正文行召回率、字符准确率和表格单元格准确率
- [ ] 能说明OCR为什么读取结构化元素而不是来源增强文本
- [ ] 能解释为什么当前不计算综合总分
- [ ] 能正确描述3份合成样本100%的证据边界

## 54. 阶段2.16.2：评价流程图和UI结构

### 54.1 流程图不能只看识别了多少文字

流程图的关键不只是“提交订单、校验库存”这些节点，还包括节点之间怎么连接，以及连接条件是什么。
因此评分同时检查：
...
只识别全部节点但漏掉失败分支时，节点召回率仍然可以是1.0，而关系准确率会降低。这能准确指出错误类型。

### 54.2 为什么额外识别也要扣分

如果UI图只有“选择文件”按钮，模型却额外生成“删除文件”按钮，这不是更多覆盖，而是无依据断言。
UI准确率使用预期数量和实际数量的较大值作为分母，因此缺失控件和编造控件都会降低分数。

### 54.3 为什么评分代码不直接调用视觉模型

评分职责是比较结构化结果和金标准；视觉模型职责是生成结构化结果。两者分开后，pytest可以使用Fake结果稳定验证评分，
真实模型则可以在有端点、预算和授权时单独运行，不会让每次单元测试产生费用和等待。

### 54.4 面试问题与参考答案

**问题：流程节点召回率100%，为什么流程理解仍可能错误？**

参考答案：节点齐全不代表连线和分支正确。模型可能把失败分支连到成功节点，或者漏掉条件，所以还要独立评价关系和条件。

**问题：当前可以宣称视觉理解效果达到要求吗？**

参考答案：不能。当前只使用Fake结构化结果验证了评分规则，没有运行真实视觉端点。可以描述“建立了可复现的视觉语义评分”，
不能描述真实识别准确率。
...
## 55. 阶段2.16.3：RAG不是“搜到了就算对”

### 55.1 四个指标分别回答什么

- Recall@K：应该找到的资产，在前K个结果里找到了多少；
- Precision@K：前K个位置里，有多少位置是相关资产；
- MRR：第一个相关资产排在第几位，第一名为1，第二名为0.5；
- forbidden_hit_rate：明确无关的资产有多少被错误召回。

例如期望资产排在第二位，`Recall@3=1`，但`MRR=0.5`。这说明它最终被找到了，却没有排在最优先位置。

### 55.2 为什么按asset_id而不是Chunk评分

Milvus搜索返回的是Chunk，但最终提供给Agent的是MySQL中的完整KnowledgeAsset。同一个资产可能命中多个Chunk，
如果直接按Chunk计数，会把同一份资产重复当成多个正确答案。因此评测先保持排序去重，再计算资产级指标。

### 55.3 为什么要标注禁止召回资产

RAG不只是担心漏掉知识，也担心错误知识污染测试分析。例如订单库存需求如果召回优惠券门槛规则，可能让模型生成无依据场景。
禁止资产命中率让这种风险成为可统计指标，而不是只看“召回数量很多”。

### 55.4 面试问题与参考答案

**问题：Recall@K高是否说明RAG效果好？**

参考答案：不一定。它可能同时召回大量无关资产，所以还要结合Precision@K、MRR和错误召回指标判断。

**问题：当前5份Fake结果能证明Milvus检索有效吗？**

参考答案：不能。Fake测试只验证金标准和指标公式。真实效果需要准备对应KnowledgeAsset，实际运行Embedding和Milvus后再统计。

### 55.5 掌握检查

- [ ] 能解释Recall@K、Precision@K和MRR的区别
...
### 55.7 真实RAG评测为什么必须显式开启

真实评测会向MySQL写入合成KnowledgeAsset、调用Embedding并向Milvus upsert向量，已经不是普通单元测试。
所以入口默认拒绝运行，只有配置`RUN_RAG_INTEGRATION_EVALUATION=1`后才执行，并在报告中标记
`real_embedding_milvus_mysql`。这样不会因为开发者误运行pytest就污染共享知识库或生成费用。

**问题：为什么真实评测种子也要保存完整KnowledgeAsset？**

参考答案：线上检索不是直接把Milvus短文本交给Agent，而是按asset_id回查MySQL完整资产并校验状态、版本和哈希。
如果评测只写向量不写权威资产，就绕过了项目最重要的错误关联控制。

### 55.8 这次真实结果说明了什么

5份合成查询的正确资产都排在第一名，所以Recall@3和MRR均为1.0。但权限查询还召回了明确禁止的订单库存资产，
使宏平均禁止命中率为0.1。这说明“找到了正确答案”与“没有混入错误知识”是两个不同问题。

真实运行还发现Fake没有暴露的SDK兼容问题：PyMilvus的Hit使用`.id`和`.distance`属性，而Fake使用字典键。
增加真实集成证据的价值不仅是得到分数，也能发现模拟对象与真实SDK之间的差异。

**问题：这组数据能否证明当前RAG适合生产？**

参考答案：不能。样本只有5份且内容简单，只能证明端到端链路可运行并暴露一处错误召回。还需要增加脱敏资产、
比较阈值和Top-K，并观察知识库规模增长后的召回质量与耗时。

### 55.9 为什么参数实验复用MySQL快照和查询向量

Top-K和阈值实验要比较的是召回过滤策略，而不是重复连接数据库或重复计算相同Embedding的速度。
所以Runner先从MySQL批量读取一次权威资产，并为每条固定查询缓存一次真实向量；每个组合仍执行真实Milvus搜索。
这既把运行时间从约40秒超时降到约8.5秒，也保证9组实验使用相同输入。

**问题：阈值0.70在5份样本上更好，为什么不直接修改线上配置？**

参考答案：5份样本太少，而且每个查询只标注1个相关资产。提高阈值可能在更复杂需求中漏掉次相关但有价值的历史资产。
当前证据只够把0.70列为候选，需要扩大脱敏数据集后再决定。

## 56. 阶段2.16.4：Reviewer不能靠“挑出很多问题”拿高分

### 56.1 为什么要同时准备缺陷样本和正确样本

如果数据集里全是错误测试点，Reviewer把每条都判为有问题也可能得到很高Recall。加入正确样本后，错误报告问题会进入FP，
同时提高正确样本误报率。这样才能同时评价“找得全”和“别乱报”。

### 56.2 三个基础计数

- TP：金标准有这个缺陷，Reviewer也准确指出；
- FP：金标准没有这个缺陷，Reviewer却报告了；
- FN：金标准存在缺陷，Reviewer没有找到。

`Precision = TP / (TP + FP)`衡量报告的问题中有多少是真的；`Recall = TP / (TP + FN)`衡量真实问题找到了多少。

**问题：为什么每份样本只注入1～2个缺陷？**

参考答案：如果一份测试点同时塞入十几种错误，很难判断Reviewer漏检是因为哪种能力不足，也难定位Reviser修复产生的副作用。
小而明确的缺陷组合更适合建立第一版可解释基线，后续再增加长集合和复杂组合。

### 56.3 为什么结构化字段优先于自由文本关键词

`requirement_coverage`明确包含需求事实和覆盖状态，`duplicate_groups`明确包含重复标题，
`hallucination_issues`明确包含问题测试点，因此可以稳定映射。`revision_suggestions`只是自然语言，
只有同时命中具体测试点标题和明确问题词时才分类，否则容易把普通优化建议误判成缺陷。

**问题：保守映射会不会降低Recall？**

参考答案：会有这种可能，但这是可观察的漏检，不会悄悄制造高分。后续真实基线可以根据FN分析是否需要扩展结构化Reviewer schema，
而不是不断堆叠模糊关键词。

### 56.4 Runner为什么接收Reviewer而不是自己判断缺陷

Runner负责把考卷转换成`TestAnalysisState`、调用Reviewer、适配输出并评分；Reviewer负责真正判断缺陷。
这样pytest可以注入Fake验证接线，真实实验也可以复用同一套指标，不会在评测代码里复制一套质量判断。

**问题：Fake报告Recall为0.9167，能说明当前模型水平吗？**

参考答案：不能。Fake答案来自金标准，只用于验证数据流。漏掉的1项说明保守适配器没有把“上传6个文件”
自动归成边界缺失，它是适配规则证据，不是DeepSeek能力结果。真实模型效果必须显式运行现有Reviewer后再报告。

### 56.5 真实Reviewer低分说明什么

12份样本中TP=4、FP=7、FN=8，说明当前Reviewer输出和金标准之间仍有明显差距。3份样本甚至在重试后
仍违反结构化类型契约，说明质量问题不仅是“有没有识别缺陷”，还包括“能否稳定返回程序可消费的数据”。

**问题：正确样本误报率为0，能否说明Reviewer不会误报？**

参考答案：不能。正确样本只有2份，其中1份结构化输出失败；样本数量也太少。必须同时报告失败率、Precision、
Recall和样本范围，不能只挑最好看的单个指标。

### 56.6 Reviser为什么还要评价副作用

Reviser可能修好一个遗漏场景，却顺手删除或改坏原本正确的测试点。如果只统计目标缺陷是否消失，就会把这种结果误判为成功。
因此第一版同时计算目标修复率和正确测试点保留率，并记录金标准之外的新标题。

**问题：Fake Reviser两项指标都是1.0，能写进简历说修复准确率100%吗？**

参考答案：不能。Fake直接使用金标准生成结果，只证明Runner和评分公式正确。真实效果必须让现有Reviser处理同一批样本，
并报告失败、未修复和副作用数据。

### 56.7 严格修复率为什么可能低估语义效果

当前金标准按完整结构精确比较。模型如果把“库存不足”改名为“库存不足拒绝创建订单”，即使业务含义接近，也会因为标题不同而未命中。
严格指标的优点是确定、可复现，缺点是无法识别同义表达，所以必须把它称为“严格目标修复率”。

**问题：保护率1.0能否证明Reviser没有副作用？**

参考答案：只能说明这6份小样本中，金标准标记的保护测试点没有被改坏。它不能覆盖测试点顺序、跨场景关系或更多复杂集合，
也不能抵消1份执行失败。面试时要同时说明样本范围和评分口径。

## 57. 阶段2.16.5：三方案为什么必须共用一把尺子

如果基础组和RAG组使用不同需求，指标差异可能来自输入难度，而不是RAG本身。统一Runner强制三组运行相同10份需求、
输出同一结构，并使用同一套金标准评分，才能把差异主要归因于方案组件。

**问题：为什么第一版先用严格文本匹配？**

参考答案：它确定、便宜且可重复，不依赖另一个LLM打分；缺点是同义表达会被判错。因此最终报告需要同时声明严格指标限制，
主观可执行性等指标再使用明确人工评分规则补充，不能偷偷用LLM改分。

### 57.1 为什么实验层读取TaskView

`TaskView`是Application Service提供的隔离副本，页面和评测代码都只能读取。三组执行器以后只要返回TaskView，
统一适配器就能提取质量结果、耗时和Token，而不需要直接操作AgentState或Repository。

**问题：为什么Token要优先用供应商值？**

参考答案：供应商usage来自真实请求，比字符估算更接近实际计费口径；当接口没有返回usage时，估算值仍可用于相对比较，
但报告必须保留来源，不能把估算Token冒充真实Token。

### 57.2 为什么三组都把问题回答为“暂不确定”

如果只给完整组补充正确业务答案，它的质量提高可能来自额外信息，而不是Reviewer/Reviser。离线实验统一把待确认项延期，
保证三组获得的信息量一致；同时限制最多两轮，避免模型反复追问导致实验无法结束。

**问题：三组差异应该有哪些？**

参考答案：基础组不使用历史资产和质量闭环；RAG组只增加历史资产；完整组再增加Reviewer/Reviser。
其他输入、问题处理和评分规则都应一致，才能进行有意义的消融比较。

### 57.3 为什么旁路节点也要写AgentEvent

基础组为了复用同一个Orchestrator，仍要经过“检索”和“评审”位置，但实际使用的是确定性旁路节点。如果不留下标记，
后续看到100分可能误以为真实Reviewer执行过。`evaluation_bypass=true`让报告和排障都能区分真实能力与实验占位。

**问题：为什么不为三组各写一套Workflow？**

参考答案：三套流程容易在节点顺序、暂停规则和状态更新上产生额外差异，破坏公平比较。依赖注入只替换被消融的能力，
其余仍复用同一个受控Orchestrator，更容易把结果差异归因于RAG和质量闭环。

### 57.4 烟测为什么不能直接变成简历提升数据

本次三组严格文本召回都是0，而且RAG组运行时旧日志输出触发GBK编码异常并降级。它证明三组链路能运行、完整组耗时和Token更高，
但不具备公平的质量比较条件。因此可以写“建立三方案评测框架”，不能写“RAG使覆盖率提升多少”。

**问题：为什么一个print会导致RAG降级？**

参考答案：检索结果包含Windows GBK无法编码的特殊字符，完整对象打印抛出异常，而打印位于检索try块中，异常被当作检索失败捕获。
修复后只记录结果数量和数值元数据，避免日志操作影响业务，也减少把完整需求内容输出到终端的风险。

## 58. 阶段2.16.6：如何把工程事实写成简历证据

简历表述要分成两层：第一层是可以由代码和测试证明的“做了什么”，例如受控编排、任务恢复、知识资产关联和离线评测；第二层是必须由真实实验支撑的“效果如何”。只有第一层完成时，可以描述能力，但不能虚构提升比例。

**问题：真实Reviewer指标不高，项目还值得写吗？**

参考答案：值得。项目价值不只来自模型分数，还来自建立了可重复的评测集、Runner、失败分类和改进依据。真实低分比只展示成功Demo更能说明测试开发中的质量意识，但简历必须如实描述为“建立基线并定位问题”，不能写成“显著提升准确率”。

**动手练习：**根据`docs/RESUME_EVIDENCE.md`，用三分钟讲清业务痛点、受控状态机、Human-in-the-loop、MySQL/Milvus分工和一项真实评测限制。

## 59. 为什么长PRD会造成结构化JSON截断

输入PRD很长不一定代表输入窗口超限。本次错误发生在输出侧：需求越长，需要返回的事实、规则和风险越多，模型在JSON闭合前达到`max_tokens`，客户端根据`finish_reason=length`明确报错。修复是给需求分析使用与其他大JSON节点一致的8192输出额度，而不是吞掉残缺JSON。

**问题：为什么不直接把max_tokens设得无限大？**

参考答案：模型和接口都有上下文限制，更大的输出还会增加耗时、成本和失控内容。8192适合当前演示规模；再大的PRD应采用分段提取与确定性合并，并保留来源，而不是继续增加单次响应长度。

## 60. 长PRD为什么使用Map-Merge

Map阶段把章节拆成有界片段，每个片段复用同一个需求分析契约；Merge阶段不再调用LLM，而由Python按原顺序合并并规范化去重。这样单次输出不会随整篇PRD无限增长，也避免“汇总调用”再次生成超长JSON。

**问题：分段分析解决了什么，又新增了什么风险？**

参考答案：它解决单次输出截断和失败定位问题，也能给风险与问题保留片段来源；但片段模型看不到其他片段的完整语义时，可能把后文已有答案当作信息缺口。当前通过最多3个问题和人工确认控制影响，后续可增加一次小输出的全局问题复核。

**问题：为什么当前不直接并行所有片段？**

参考答案：顺序执行更容易验证状态、错误和外部限流行为。真实烟测已经证明耗时会增长；只有在正确性稳定后，才适合增加2～3路有界并行，并需要同时处理限流、超时和结果顺序。

## 61. 自适应拆分为什么是稳定性保护而不是性能优化

片段输出截断后继续减半，可以避免整份任务立刻失败；但被截断的那次调用已经消耗了时间和Token，随后还要执行多个子片段。因此它提高完成概率，却可能显著增加总耗时。真实Flash复验用了8次调用和319.55秒，正说明“能完成”和“体验合格”是两个不同指标。

**问题：为什么低温度仍然不能解决截断？**

参考答案：温度0减少随机性，但不会改变模型必须输出多少事实和规则，也不会取消服务商的推理与输出预算。根治方向是缩小输出协议，例如让模型只返回statement_id分类，而由Python回填原文。

**问题：什么时候才应该触发自适应拆分？**

参考答案：只有供应商明确返回`finish_reason=length`或等价截断错误时。网络超时、非法字段和业务校验失败具有不同根因，盲目拆分只会重复请求并掩盖问题。

## 62. 稳定ID为什么能减少长PRD输出

旧方案让模型在每个分片里重复输出完整事实、规则、状态、风险和问题。输入越长，输出也越长，结构化JSON容易在结尾被截断。

新方案先由Python把原文拆成稳定陈述：
...
模型只需返回`fact_ids=[S001,S002]`、`business_rule_ids=[S002]`。Python再从目录中取回原文，因此既减少输出Token，也不会让模型改写数字。模型若返回不存在的ID，校验层会直接拒绝。

### 为什么问题要全局审核

分片A可能不知道答案在分片B，容易提出“原文其实已回答”的问题。先分类全部陈述，再用一次全局调用审核阻塞问题，可以看到整份需求，并控制最多5个关键问题。

### 为什么结构化调用关闭思考模式

DeepSeek V4默认开启思考模式，推理内容也会消耗`max_tokens`。对于“按固定Schema分类ID”这种任务，目标是短小、合法、可校验的JSON。显式关闭思考模式后，同一演示PRD从约5分钟且截断，变为13.38秒完成。普通创作型文本没有被统一关闭思考。

### 面试问题

**问：为什么不只把`max_tokens`继续调大？**

参考答案：调大上限没有减少重复内容，也会增加等待和成本。稳定ID从数据契约上减少输出，并让Python负责来源回填和非法ID校验；输出上限只保留为最后的有界保护。

**问：13.38秒是否证明所有长PRD都解决了？**

参考答案：不能。它只是一份2735字符合成PRD的真实证据。还要用更多长度、领域和图文样本评估事实/规则召回、待确认问题准确性和耗时分布。

### 动手练习

- [ ] 找到`RequirementStatementExtractor`，说明`statement_id`为什么稳定
- [ ] 找到`CompactRequirementBatch`，说明未知ID为什么必须拒绝
- [ ] 找到全局问题Prompt，说明它如何避免重复询问
- [ ] 找到`DeepSeekClient`中的`thinking`参数，说明只对结构化调用生效的原因

## 63. 为什么下游节点不能重复携带全部长PRD分析结果

需求分析已经把原文转换为事实、规则、状态和风险。如果知识检索又同时携带完整原文和全部结构化结果，同一句业务规则可能出现两到三次，不但浪费Embedding输入，还会超过节点预算。

本次修复没有删除AgentState中的完整数据，而是只创建一个有界“检索视图”：原始需求、事实、规则、状态和风险各有独立额度；包含金额、数字、权限、状态和幂等等信息的条目优先进入查询。被裁剪的分区记录在ContextMetrics中，方便解释为什么本次检索只使用了部分信息。

**问题：为什么不直接把知识检索预算从4000调到20000？**

参考答案：向量检索需要的是能代表业务语义的查询，不是重复的完整任务快照。单纯抬高预算会增加Embedding耗时和噪声，也无法限制未来更长PRD。分区预算使输入规模稳定，同时保留完整领域状态供后续生成和评审使用。

**问题：裁剪会不会丢失关键规则？**

参考答案：存在风险，因此规则分区额度高于普通事实，并优先选择含数字、状态、权限、时限等提示的条目；同时记录裁剪指标。是否影响召回质量仍需通过RAG离线评测验证，不能只凭代码推断。

## 64. 结构化输出兼容为什么必须保守

模型可能把“问题列表”偶尔返回成对象或字符串。兼容层只能转换语义明确的形式，不能凭空补业务结论。特别是幻觉问题即使只返回字符串，也必须保留为严重问题并阻止Reviewer通过；完全未知的对象结构仍应报错。

## 65. 为什么内部Embedding请求不应误走开发代理

`requests`默认读取HTTP代理环境变量。如果Streamlit由开启本地代理的终端启动，自有Ollama请求也可能被发送到代理端口，表现为服务超时。对明确配置的内部服务使用独立Session并关闭环境代理，可以让请求目标和配置保持一致；公网LLM客户端不应被一并修改。

## 66. 为什么只提高Reviewer输出额度

第二轮评审的输出包含每条需求事实的覆盖结论，规模明显大于修正操作列表。当前真实样本在8192处截断，因此只为Reviewer设置16384的有界额度；这不是通用扩容。若未来继续增长，应把事实和测试点标题替换为稳定ID，减少重复文本。
...
## 70. 为什么前端不应该直接解析完整AgentState

完整AgentState面向后端恢复和业务执行，字段多且会继续演进。如果Vue直接根据`current_step`、事件和评审对象自行拼出页面状态，前后端就会各自维护一套展示规则。

`GET /progress`提供一个小型只读模型：原始状态值便于程序判断，中文标签直接展示，后台执行状态说明线程是否仍在运行，关键计数用于任务概览。最近事件只返回3条，避免轮询响应越来越大。

需要特别区分两种状态：

- `status`是Agent业务状态，例如`waiting_for_user`或`completed`；
- `execution_status`是后台Runner状态，例如`running`或`stopped`。

任务等待用户时Runner应该停止，但任务并没有失败。前端同时读取这两个字段，才能给出正确提示。
...
只测试“创建任务返回201”无法证明Agent后端可用。这个项目的关键能力是Human-in-the-loop，因此接口验收必须经历：后台任务主动暂停、用户提交补充、同一个`task_id`恢复，以及完成后读取报告。

测试中的脚本化Orchestrator只替代慢且不稳定的模型输出，Application Service、Runner、Repository和HTTP路由都是真实代码。这样既能快速重复运行，又能证明前后端边界没有断裂。

### 面试问题与参考答案

**问：Fake链路通过是否代表真实模型效果合格？**

参考答案：不代表。它证明工程控制流、持久化边界和HTTP契约正确；模型质量必须由离线金标准和真实集成实验分别证明。项目不能把两类证据混在一起。

## 73. 为什么原生前端仍然拆成三个文件

单个HTML确实可以同时写CSS和JavaScript，但任务交互继续增加后会很难阅读。当前只拆为三个稳定职责：

- `index.html`负责页面结构和无障碍标签；
- `styles.css`负责布局和视觉；
- `app.js`负责API调用、轮询和页面状态。

这仍然是无框架前端，不需要Node.js或构建工具。FastAPI同源托管后，浏览器直接请求相对路径API，也暂时不需要处理CORS。

### 面试问题与参考答案

**问：为什么先不用Vue？**

参考答案：当前前端只服务于后端Agent能力演示，状态和页面数量有限。原生实现能更快验证API契约并降低学习成本；当历史任务、复杂路由和组件复用成为真实问题时，再迁移框架更合理。

## 74. 原生JavaScript如何实现Human-in-the-loop

前端轮询到`waiting_for_user`后停止定时器，再读取任务详情中的`open_questions`。每个问题必须填写文本或选择“暂不确定”；提交后先调用`clarifications`保存答案，再调用`run`恢复后台执行。

前端只负责表单校验和用户动作，后端仍会再次校验问题集合是否完全匹配、答案是否为空以及任务是否真的处于等待状态。因此页面校验提升体验，Application Service校验保证业务安全，两者不能互相替代。

### 面试问题与参考答案

**问：为什么问题文本可以直接作为answers对象的key？**

参考答案：这是当前后端契约，Application Service会把答案key集合与当前`open_questions`严格比较。更复杂产品可以为问题增加稳定ID，但当前阶段没有第二种问题模型，不为形式提前扩展领域结构。

## 75. 前端分页为什么不能修改AgentState

测试点分页只是浏览方式。前端保留后端返回的完整测试点数组，根据`(当前页-1)×5`进行切片；翻页不会调用修改接口，也不会重排或覆盖Repository中的测试点。

详情Dialog直接接收当前测试点对象，展示前置条件、步骤、预期结果和来源。页内序号只帮助用户阅读，不作为人工反馈或后端更新的目标ID。

### 面试问题与参考答案

**问：为什么不用后端分页接口？**

参考答案：当前单任务测试点规模有限，完整任务详情本来就需要供报告和评审页面使用。先做纯展示层分页更简单；只有真实任务规模或网络测量证明响应过大时，才增加服务端分页，避免提前制造两套结果读取契约。

## 76. 为什么质量评审和最终报告不需要新增API

任务详情本来就包含`review_result`和`report`。前端在测试点结果版本变化时只读取一次完整任务，再分别交给测试点、质量评审和报告三个渲染函数，避免同一个页面连续请求三次相同快照。

Reviewer结果是结构化对象，因此可以直接展示总分、4个维度和问题列表。最终报告是Markdown文本，当前演示版使用`textContent`放进`pre`标签，不把模型输出当HTML执行；下载时由浏览器创建UTF-8 Blob，不需要后端再生成临时文件。

### 面试问题与参考答案

**问：为什么不在前端直接把Markdown转换成HTML？**

参考答案：模型报告包含外部数据，直接拼接`innerHTML`会增加脚本注入风险。当前展示目标只需要可读和可下载，纯文本预览最小且安全；未来如需富文本，应引入受维护的解析器并对输出做严格清理。

**问：切换结果导航为什么不会重新执行Agent？**

参考答案：导航只修改前端的`activeResultTab`并切换三个结果区的显示状态，不调用`run`或`advance`接口。Agent执行权仍只在Application Service和Orchestrator链路中。

## 77. 为什么业务规则反馈需要二次确认

测试建议只影响测试点，可以直接交给Reviser处理；业务规则会进入正式需求事实，如果把用户随手输入的建议立即当作确定规则，后续测试点和报告都会建立在未经确认的前提上。

因此业务规则提交后先保存为`pending_confirmation`。前端只展示确认入口；用户确认后Application Service才把它写入业务规则，取消则标记为`rejected`。随后前端调用统一的`run`入口，具体执行Reviser、Reviewer还是Finalizer仍由Orchestrator判断。

### 面试问题与参考答案

**问：为什么前端确认规则后还要调用`run`？**

参考答案：确认接口负责保存用户决定和更新任务状态，后台Runner负责让长耗时Agent链路脱离当前HTTP请求继续执行。两者职责不同，节点选择始终在Orchestrator中。

**问：人工反馈为什么不能使用当前分页序号作为目标？**

参考答案：翻页或修正后页内序号可能变化。当前契约使用完整任务中的真实测试点标题作为目标，页面序号只用于阅读，不能承担业务身份。

## 78. 为什么端到端验收仍然使用Fake Orchestrator

验收目标是证明HTTP、后台执行、Application Service、Repository和Human-in-the-loop状态能够正确连接。真实LLM输出慢且不稳定，不适合作为每次提交都运行的回归测试，因此只把Orchestrator节点结果脚本化，其他工程边界都使用真实代码。

这类测试不能证明模型生成质量，只能证明控制链路。需求事实准确率、风险覆盖率和测试点质量仍必须由离线金标准评测证明。

### 面试问题与参考答案

**问：DOM契约测试能否代替真实浏览器测试？**

参考答案：不能。它只能发现JavaScript引用了不存在的HTML元素，无法验证点击、布局、下载和浏览器兼容性。当前同时保留HTTP链路测试，并把真实浏览器手动验收列为明确限制，不把缺少的证据说成已完成。

**问：为什么人工反馈验收要检查同一个task_id？**

参考答案：反馈是对现有测试分析结果的修正。如果创建新任务，原测试点、评审历史、修正次数和业务规则上下文都会断开，无法证明任务暂停恢复和持久化边界正确。

## 79. 为什么恢复后台任务时不能只看Agent业务状态

提交补充信息后，HTTP接口先把后台任务放入队列。线程真正开始执行前，Agent业务状态可能短暂仍是`waiting_for_user`，但后台状态已经是`queued`或`running`。如果前端只看到`waiting_for_user`就停止轮询，会永远停在旧页面。

因此停止条件必须同时判断两层状态：业务确实在等待用户，并且后台已经不在排队或执行。这里不是修改Agent状态机，而是正确解释异步执行的短暂状态组合。

Reviewer面对大量事实时也不应逐字往返传输。输入仍保留事实原文供模型判断，但输出只返回`F001`等稳定ID；Python再恢复原文并校验是否覆盖全部事实。这样缩短结构化输出，同时不改变最终领域数据。

### 面试问题与参考答案

**问：为什么不直接把Reviewer的max_tokens继续调大？**

参考答案：无界提高额度会增加延迟和成本，也受模型服务端上限约束。先减少缩进和重复文本、限制问题数量，并用稳定ID代替重复事实原文；只有明确截断时再受控重试一次，边界更可解释。

**问：事实ID会不会污染AgentState？**

参考答案：不会。ID只用于Reviewer请求和响应传输，Python在构造`TestPointReviewResult`后立即映射回原事实，再执行原有覆盖校验，保存到状态和数据库的仍是事实原文。

## 80. 结构化输出为什么可以忽略额外字段

严格JSON校验不等于所有对象都必须“一个字段也不能多”。对于Reviewer幻觉问题，真正影响业务判断的是测试点标题、问题说明和无依据断言。模型偶尔附带来源定位等额外信息时，只要三个核心字段完整且类型正确，就可以提取核心字段并丢弃额外信息。

这不是无条件容错：缺少`unsupported_claim`等核心字段仍然失败，分数、需求覆盖和幻觉门禁也没有放宽。原则是对业务语义严格、对无害的传输冗余宽容。

### 面试问题与参考答案

**问：为什么不把模型返回的所有额外字段都保存下来？**

参考答案：未进入正式契约的字段含义和稳定性无法保证，持久化后反而会扩大快照兼容成本。当前只提取经过校验的领域字段，额外内容只用于本次解析并被丢弃。

## 81. 为什么来源标签需要归一化但不能无限容错

测试点来源是固定领域枚举，但模型可能把同一语义写成上下文分区名，例如把推导风险写成`inferred_risk`、把历史资产写成`rag`。这些值可以确定性映射到`requirement`和`historical_asset`，归一化不会改变业务含义。

未知值不能自动归为需求来源，否则可能把没有依据的互联网知识或模型猜测伪装成正式需求。系统只接受白名单中的明确同义词，其他值仍报错，并把非法标签写进错误信息便于定位。

### 面试问题与参考答案

**问：为什么不直接删除sources字段？**

参考答案：来源用于Reviewer检查历史资产是否污染当前需求，也是报告可追踪性的证据。正确做法是保持领域枚举稳定，在LLM传输边界归一化明确同义词，而不是放弃来源校验。

## 82. 为什么历史列表不能直接读取所有完整快照

`agent_tasks.snapshot_json`保存完整AgentState，可能包含大量测试点、事件和报告。历史列表只需要任务ID、状态、摘要和更新时间，如果让MySQL排序并返回全部JSON，不仅网络传输浪费，还可能因为排序大字段耗尽数据库排序内存。

因此Repository提供独立的摘要分页契约：先用小字段搜索和分页，用户点击某条记录后才按`task_id`读取完整快照。页面恢复的是原任务，不复制任务；终态只读，运行态才继续轮询。

### 面试问题与参考答案

**问：历史任务列表和Agent快照分别解决什么问题？**

参考答案：摘要列表负责低成本地“找到任务”，版本化快照负责完整地“恢复任务”。二者共用`task_id`关联，但读取粒度不同。

**问：为什么恢复完成任务后不能自动调用run？**

参考答案：完成任务是终态，自动run会带来节点重复执行风险。页面先读取进度，只对后台仍处于queued或running的任务恢复轮询，业务执行权仍在后端。

## 83. 知识库保存为什么分成MySQL和Milvus两步

用户点击保存后，系统先做准入校验：任务必须完成、Reviewer通过、没有未解决问题和待处理反馈，且最终报告没有过期。通过后，MySQL保存完整、版本化的知识资产，这是权威内容；随后系统只把有界短片段和关联标识写入Milvus，用于相似度召回。

Milvus命中后通过`asset_id + asset_version + content_hash`回到MySQL核验完整内容。因此Milvus负责“快速找到候选”，MySQL负责“提供可信原文”，不是两份互不关联的数据。

### 面试问题与参考答案

**问：为什么知识资产不能在任务完成后自动保存？**

参考答案：需求和测试结果可能含公司敏感信息，且用户未必希望复用。显式点击和数据安全勾选形成可审计的Human-in-the-loop准入，不把生成成功等同于允许沉淀。

**问：Milvus索引失败时为什么不删除MySQL资产？**

参考答案：MySQL已经保存权威资产，删除会丢失用户确认结果。系统将状态标记为`index_failed`并明确提示，后续可以幂等重试索引。

## 84. 为什么任务命名不需要再调用一次LLM

多数PRD已经有Markdown标题、文档标题或首行需求名称。为了生成一个导航标签再增加LLM请求，会引入额外耗时、费用和不稳定输出，收益很低。当前使用确定性规则提取标题，并把它作为MySQL摘要列保存。

任务名不写入AgentState，因为它不参与状态机和模型推理；它属于列表查询和文件下载使用的展示元数据。旧任务没有该列值时用已经保存的需求摘要回填，避免重新执行历史任务。

### 面试问题与参考答案

**问：为什么任务名和需求摘要要分开保存？**

参考答案：任务名需要短、稳定，服务于导航和文件名；需求摘要由RequirementAnalyzer生成，内容更长且可能在分析后才出现。混用会导致任务标题随执行阶段变化。

**问：报告文件名为什么需要额外清理？**

参考答案：需求标题可能包含斜杠、冒号等Windows禁用字符。下载前替换这些字符可以保证生成的文件名在常见操作系统中可保存。

## 85. 删除任务为什么不直接删除知识资产

历史任务删除针对的是工作区记录：任务快照、Agent事件和执行记录。知识资产则经过用户额外确认、版本化并可能已经被其他任务召回。如果删除历史导航时顺便删除知识资产，会让知识库内容在没有明确提示的情况下消失。

因此任务表依靠外键级联清理自己的事件和执行记录，知识资产需要未来在知识库管理入口中独立下线。悬浮确认层只改变交互方式，真正删除仍通过后端Application Service完成。

## 86. 为什么滚动容器里的浮层会被遮挡

历史列表需要`overflow-y: auto`才能独立滚动，但它也会裁剪超出列表边界的子元素。即使子元素设置很大的`z-index`，也无法越过祖先元素的裁剪区域。

解决方法是把确认层挂载到`body`，使用删除按钮的`getBoundingClientRect()`计算视口坐标，再用`position: fixed`定位。这样浮层与历史列表不在同一个裁剪上下文。工作区高度同样基于视口计算，长内容由面板内部滚动，避免浏览器页面无限变长。
## 87. 为什么任务重命名不修改AgentState

任务名称服务于历史导航和下载文件名，不参与需求分析、节点决策或质量评审。如果为了改一个标题而修改AgentState快照，会扩大领域状态、序列化格式和并发保存的影响范围。

因此页面只提交“重命名任务”这个用户动作，Application Service确认任务当前没有运行，再由Repository更新独立的`task_name`列并递增版本。这样MySQL中的完整AgentState保持不变，重新打开任务时仍能看到新名称。

### 面试问题与参考答案

**问：为什么运行中的任务不允许重命名？**

参考答案：节点执行结束会保存任务快照和版本，重命名同时更新版本可能产生竞争。当前V1选择在执行中禁用重命名，用简单边界避免覆盖；未来如有强需求，可以让重命名使用独立元数据版本或完整乐观锁重试。

**问：“新建任务”为什么不立即向数据库插入一条记录？**

参考答案：点击入口只表示用户准备输入新需求，此时还没有有效需求。页面先重置为输入态，只有用户提交文本或文件后才调用创建任务接口，避免数据库产生大量空任务。

## 88. 为什么修正上限不能只根据AgentStatus展示

自动修正达到上限后，Orchestrator已经返回`revision_limit_reached`并停止后台执行，但领域状态仍允许用户提交人工反馈，因此不能简单改成`completed`或`failed`。如果页面只读取`AgentStatus.RUNNING`，就会把“等待人工介入”误显示成“模型仍在执行”。

正确做法是保留领域状态，由Presenter结合最后决策和后台执行状态生成用户文案；前端将修正上限作为轮询出口，并引导用户进入人工反馈。这样既不破坏反馈恢复规则，也不会伪造运行状态。

### 面试问题与参考答案

**问：为什么不直接把达到修正上限的任务改成失败？**

参考答案：质量评审未达门槛不等于技术执行失败，系统已经产出测试点并等待测试人员补充意见。把它标为失败会丢失Human-in-the-loop语义，也可能让重试入口错误地创建新任务。

## 89. 为什么产品名不直接使用Agent

“Agent”描述的是内部实现方式，不是用户最关心的结果。当前产品实际覆盖需求理解、历史资产检索、测试点生成、质量评审、人工反馈和报告沉淀，因此“AI 测试分析助手”比“需求分析助手”更完整，也比“平台”更克制。

面试时可以区分两句话：产品定位是AI测试分析助手；技术实现是由Python Orchestrator控制的Agentic Workflow。这样既不会把产品能力缩小为需求抽取，也不会把受控流程夸大成完全自主Agent。

## 90. 为什么知识库列表不能直接读取完整资产

`knowledge_assets.asset_json`保存原始需求、结构化需求、测试点、评审和最终报告，适合恢复单条资产，但不适合列表批量读取。知识库列表只需要摘要、状态、评分、测试点数量和时间，因此Repository提供独立摘要分页查询；用户点开某条记录时，详情接口才按`asset_id`读取完整快照。

### 面试问题与参考答案

**问：Milvus命中资产后，为什么还要回MySQL？**

参考答案：Milvus负责用短片段和向量找到候选，并返回`asset_id`等关联标识；MySQL保存完整且版本化的权威资产。候选命中后回查MySQL，才能得到完整测试点和报告，并校验状态、版本和内容哈希。

## 91. 为什么知识库使用独立页面

任务工作台围绕单个PRD的输入、执行和结果展开；知识库围绕跨任务资产的搜索、筛选和复用展开。把知识库塞进任务工作台会让三栏继续变窄，也会混淆“当前任务结果”和“已确认历史资产”。因此顶部只保留两个稳定入口，知识库内部采用摘要列表加详情阅读区。

### 面试问题与参考答案

**问：为什么知识库列表和详情使用两个接口？**

参考答案：列表需要高频分页，只读取小字段；详情需要低频读取完整JSON。拆开后可避免搜索时反复传输原需求、测试点和整份报告，也使MySQL索引能直接服务列表查询。

## 92. 为什么知识资产管理优先使用停用而不是删除

知识资产可能已经被历史任务召回，也包含用户确认、版本和索引审计。物理删除一旦出现MySQL成功但Milvus失败，就会留下无法追踪的孤立向量；反向失败则可能丢失权威内容。停用先在MySQL保留`retired`状态，再清理向量，既停止召回又允许恢复。

### 面试问题与参考答案

**问：恢复已停用资产为什么不能只把状态改回indexed？**

参考答案：停用时已经清除了Milvus向量，只改数据库状态会让系统声称“已索引”但实际无法召回。恢复必须先转为`pending_index`，重新Chunk、Embedding和upsert，成功后才写回`indexed`。

## 93. 为什么图文PRD评测要分解析层和语义层

DOCX中存在图片，不等于系统已经理解图片。解析层先验证正文、表格和图片附件是否完整进入`DocumentContent`；语义层再验证视觉模型是否正确识别流程节点、分支条件和UI控件。两层分开后，失败时才能判断是“图片没有提取”还是“模型理解错误”。

### 面试问题与参考答案

**问：合成图文PRD能证明真实企业文档效果吗？**

参考答案：不能完全证明。合成样本的价值是公开、可重复、有金标准，适合持续回归；真实效果还要用经过授权和脱敏的企业样本补充验证。简历中应写“在自建脱敏图文样本集上验证”，不能声称覆盖全部企业PRD。

**问：为什么不把两份综合PRD直接加入需求分析金标准？**

参考答案：它们首先验证文档解析和多模态输入是否完整，而`seed_v1.json`验证事实、规则、风险和问题的模型质量。分开统计可以避免解析失败污染需求分析指标，也避免一个总分掩盖问题来源。

## 94. 为什么低分报告可以下载但不能进入知识库

报告是否可用和资产是否可信是两个不同门槛。Reviewer经过两轮自动修正仍未达标时，测试点已经可以供测试人员参考，因此系统生成带明确风险提示的报告；但这类结果不应成为后续RAG的权威历史依据，所以知识资产准入仍要求Reviewer通过。

### 面试问题与参考答案

**问：允许低分结果最终化是否放松了质量控制？**

参考答案：没有跳过评审，也没有取消自动修正。系统保留分数、未解决问题和风险标记，只是不再强制用户提交反馈。严格门禁继续用于知识库准入，普通报告和可信资产采用不同质量等级。

**问：完成后提交人工反馈为什么仍是同一个任务？**

参考答案：`reopen_for_feedback`清除旧报告和最终汇总，但保留原始需求、测试点、评审历史和task_id。Reviser只应用本次反馈，随后重新评审并生成新版，能够完整追踪报告演进。

### 补充：结构正确不代表增量操作可以应用

Reviser返回的JSON可能字段和类型都正确，但`add`标题已经存在、`replace`目标不存在，仍然不能安全合并。校验必须覆盖“解析结构”和“应用语义”两层，并让两层共享同一个有界重试；否则模型第一次出现可修复的语义错误就会导致整个任务失败。

**问：为什么不能直接忽略重复新增？**

参考答案：同一批操作中可能还有其他修改，静默忽略会让系统错误地声称反馈已完整应用。当前保持原子语义：全部操作合法才一次性写入；不合法时整体重试或整体失败。

## 95. 为什么摘要列表正常，任务详情却可能返回500

历史摘要读取的是MySQL独立列，不需要反序列化完整AgentState；任务详情和进度则必须读取`snapshot_json`并恢复领域对象。因此新增报告字段后，如果写入端和严格读取契约没有同步，就会出现“列表看得到、详情打不开”。

本次`quality_summary.review_passed`属于向后兼容的可选布尔字段：旧快照可以没有，新快照存在时必须为布尔值。它不需要提升schema版本，因为没有改变旧字段语义；未知的其他字段仍拒绝，避免拼写错误或意外数据被静默接受。

### 面试问题与参考答案

**问：为什么不直接允许`quality_summary`中的所有未知字段？**

参考答案：完全放开会掩盖字段拼写错误和未评审的数据变化，使持久化任务恢复后的业务语义不可预测。更安全的策略是显式加入已知的兼容字段，并分别测试新快照往返、旧快照恢复和非法类型拒绝。

## 96. 为什么质量Tab有88分，任务概览却显示待评审

同一份领域结果会经过不同展示适配层。质量Tab直接读取`review_result.overall_score`，进度接口此前却读取旧字段`score`，因此真实评审已经完成，但概览Presenter返回`null`，前端回退为“待评审”。

修复应放在API Presenter，而不是在前端从详情接口猜测评分，也不能改写Reviewer领域模型。Presenter优先读取正式字段`overall_score`，同时兼容旧任务可能保留的`score`。质量问题编号则属于纯展示语义，使用有序列表即可，不改变问题内容和Reviewer逻辑。

### 面试问题与参考答案

**问：为什么端到端验收仍然需要自动化测试？**

参考答案：真实验收能发现跨LLM、数据库、向量库和浏览器的集成问题，但速度慢且结果受外部环境影响。修复后要把稳定契约沉淀成Fake或静态测试，例如评分字段映射、列表结构和下载文件名，日常回归不用重复调用真实服务。

**问：如何准确描述本次97秒耗时？**

参考答案：它是单次真实样本、当前模型与网络环境下的观测值，不是性能承诺。通过节点埋点可说明主要耗时来自LLM和Milvus，并为后续优化提供基线，但简历中不能写成平均耗时或性能提升比例，除非经过多样本重复测量。

## 97. 为什么README不能继续按开发阶段罗列功能

开发日志面向维护者，需要记录每个阶段为什么修改；README面向第一次看到项目的人，需要在很短时间内回答“解决什么问题、怎么实现、有什么证据、如何运行、有哪些边界”。如果把几十个阶段全部堆入README，真正有价值的Orchestrator、HITL、RAG、持久化和评测反而会被淹没。

因此README只保留稳定能力和可验证证据，阶段顺序、踩坑与修复原因进入`DEVELOPMENT_LOG.md`，代码学习和面试追问进入`LEARNING_NOTES.md`。

### 面试问题与参考答案

**问：这个项目为什么叫受控Agentic Workflow，而不是自主Agent？**

参考答案：LLM负责需求理解、测试点生成和质量评审，但节点顺序、暂停条件、最大修正次数、数据库写入和知识准入都由Python代码控制。它具备状态共享、动态分支、工具边界和Human-in-the-loop，但没有让LLM自由规划任意步骤，因此受控Agentic Workflow更准确。
