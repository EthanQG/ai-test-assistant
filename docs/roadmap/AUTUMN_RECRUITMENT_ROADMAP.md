...
- **单人复核已完成（2026-08-06）**：schema v1契约、标注指南、10份完全虚构需求和严格校验测试
- 准备 10～20 份脱敏需求
- 标注事实、规则、关键问题、风险、必要场景和禁止断言
- 包含正文、表格、扫描页、流程图、UI截图、图文冲突和低清图片样本
- 仍需补齐真实图文附件；当前没有双人标注一致性数据，后续结论必须注明单人复核范围

### 2.16.2 图文解析评测

- **第一小步已完成（2026-08-07）**：5份脱敏合成PDF/DOCX/PNG、schema v1解析金标准和确定性样本测试
- **第二小步已完成（2026-08-10）**：正文行召回、字符准确率、表格单元格准确率和逐样本JSON报告
- **第三小步已完成（2026-08-10）**：流程节点召回、分支关系准确率和UI元素准确率；pytest使用Fake视觉结果
- 正文提取完整率、表格字段准确率和OCR文字准确率
- 流程节点召回率、分支关系准确率和UI操作识别准确率
- 关键图片漏识别数、关键问题召回率和不必要提问数量
- 文档总解析耗时、OCR耗时、视觉调用次数和失败率
- 当前仍缺少真实视觉端点结果和复杂脱敏样本，简单合成样本不能替代真实PRD验证
...
- **第四小步已完成（2026-08-10）**：9组Top-K/阈值真实对比；0.70在当前样本消除禁止命中但不直接改线上参数
...
- **第二小步已完成（2026-08-10）**：现有Reviewer结构化字段到六类稳定缺陷的保守映射
- **第三小步已完成（2026-08-10）**：fixture转换为`TestAnalysisState`，经可注入Reviewer边界、缺陷适配器和评分器生成Fake接线报告
- **真实基线已完成（2026-08-10）**：`deepseek-v4-pro`运行12份合成样本，Precision=0.3636、Recall=0.3333，3份结构化输出失败；结果不代表泛化能力
- **Reviser第一小步已完成（2026-08-10）**：6份最小修复样本，分别评价目标缺陷修复率和正确测试点保留率；当前仅Fake接线证据
- **真实Reviser基线已完成（2026-08-10）**：6份样本严格目标修复率0.1667、保护率1.0、1份结构失败；严格全字段匹配不等同人工语义正确率
- 向测试点注入遗漏、重复、幻觉、模糊预期和缺失来源
- 计算 Reviewer 缺陷检测 Precision、Recall、误报和漏报

### 2.16.5 三组消融实验

- **第一小步已完成（2026-08-10）**：固定三组名称、相同10份输入、统一输出以及质量/耗时/Token口径，Fake验证10×3矩阵
- **第二小步已完成（2026-08-10）**：将Application Service只读`TaskView`适配为统一实验输出，不让评测层直接修改领域状态
- **第三小步已完成（2026-08-10）**：三组只改变RAG和质量闭环开关，待确认问题统一按“暂不确定”恢复同一任务
- **第四小步已完成（2026-08-10）**：使用显式NoKnowledge和QualityBypass依赖装配三组，复用原Orchestrator并保留旁路事件
- **真实烟测已完成（2026-08-10）**：1份需求完成三组运行并记录耗时/Token；严格文本指标和降级RAG不足以支持质量提升结论，完整10×3降为可选增强
...
- **已完成（2026-08-10）**：新增`docs/RESUME_EVIDENCE.md`，汇总真实代码、测试、评测证据、简历边界和面试讲解

### 2.16.7 长PRD分段需求分析

- **已完成（2026-08-10）**：章节感知分段、逐段结构化分析、Python合并去重、片段失败定位和来源标签
- **真实烟测**：2735字符、2片段、222.75秒，结构化JSON未再截断
- **后续可选优化**：跨片段候选问题语义复核、2～3路有限并行；不阻塞当前秋招演示

### 2.16.8 长PRD截断稳定性保护

- **已完成（2026-08-11）**：JSON低温度、单片段字段数量约束、仅截断错误触发的最多3层自适应拆分
- **真实复验**：Flash 319.55秒完成（8次尝试、2次拆分）；Pro 480秒内未完成
- **结论**：能够降级完成不等于用户体验合格；下一步优先评估statement_id紧凑输出，不能继续单纯抬高Token或增加无界重试

### 2.16.9 长PRD紧凑ID分析

- Python提取稳定需求陈述ID，保留章节、分片和字符范围；LLM只返回分类ID和少量带依据风险
- Python按ID回填原文，未知ID严格拒绝；待确认问题改为整份需求一次全局审核
- DeepSeek V4结构化JSON调用显式关闭默认思考模式，普通文本调用保持原配置
- 同一2735字符样本真实验证由319.55秒、8次调用/2次拆分变为13.38秒、3次调用/0次拆分
- 当前只完成单样本性能与稳定性证据，语义分类质量仍需离线金标准扩样验证
...
- 至少10份脱敏需求具备金标准和离线评分能力；完整10×3真实模型实验作为可选增强，不阻塞秋招投递
- README 只展示真实结果和明确限制
- 全量离线自动化测试通过
