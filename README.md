...
- 原生Web已提供独立知识库浏览页，可搜索、按索引状态筛选、分页并查看结构化需求、测试点和最终报告；管理动作尚未实现
...
顶部“知识库”入口打开`http://127.0.0.1:8000/app/knowledge.html`，支持资产摘要搜索、索引状态筛选、分页和详情浏览。本阶段为只读页面，资产停用、删除和索引重试尚未开放。

桌面端工作区会使用浏览器可用宽度，历史任务标题最多展示两行。自动修正达到上限后页面停止轮询并进入人工反馈，不会继续显示为模型执行中。
