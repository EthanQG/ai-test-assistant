...
长PRD需求分析采用章节感知Map-Merge：约1800字符一段，各段有界生成结构化JSON，Python按原顺序合并去重并保留风险/问题的片段来源。2735字符演示PRD真实分析为2段、222.75秒且未再截断；当前片段顺序执行，候选问题仍可能出现跨片段重复确认。

结构化JSON调用固定使用`temperature=0`，需求分析Prompt限制单片段字段数量；片段仍被服务商截断时最多递归拆分3层并记录尝试次数。稳定性复验中Flash通过8次调用、2次拆分在319.55秒完成，Pro在480秒内未完成，因此该机制属于失败保护，不代表同步长PRD体验已经达到理想水平。
